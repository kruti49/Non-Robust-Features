...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/robustckpt.pth')
    model.load_state_dict(checkpoint['model'])
    best_acc = checkpoint['acc']
    start_epoch = checkpoint['epoch']

#optimizer and loss
adversary = L2PGDAttack(net, loss_fn=nn.CrossEntropyLoss(), eps=0.5, nb_iter=10, eps_iter=0.05, rand_init=True, clip_min=0.0, clip_max=1.0, targeted=False)
...
def train(epoch):
    ...
    print('Total adversarial train Accuarcy:', 100.0 * adv_correct / total)
    print('Total adversarial Loss:', adv_loss)
